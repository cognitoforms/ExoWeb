...
namespace ExoWeb
{
	...
	public class JavaScriptExpressionTranslator 
	{
		...
		static readonly Dictionary<string, string[]> supportedMembers = new Dictionary<string, string[]>()
		{
			...
			{ "Date.DayOfYear as Number", new string[] {@"Date_dayOfYear({0})", "Date_dayOfYear", @"function Date_dayOfYear(d) { return Math.round(((d - d.getFullYear()) / 1000 / 60 / 60 / 24) + .5, 0); }"} },
			...
			{ "static Date.Now as Date", new string[] {@"new Date(Date.now())", "", @""} },
			{ "static Date.Parse(String) as Date", new string[] {@"Date.parse({0})", "", @""} },
			{ "static Date.Today as Date", new string[] {@"new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate())", "", @""} },
			{ "static Date.UtcNow as Date", new string[] {@"Date_toUTC(new Date(Date.now()))", "Date_toUTC", @"function Date_toUTC(d) { return new Date(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate(), d.getUTCHours(), d.getUTCMinutes(), d.getUTCSeconds(), d.getUTCMilliseconds()); }"} },
			...
			{ "TimeSpan.Ticks as Number", new string[] {@"({0}.milliseconds*10000)", "", @""} },
